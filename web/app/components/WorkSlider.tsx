...
// Horizontal swipe recognition: locks to an axis after a few pixels of
// movement, tracks the drag offset for the live transform, and calls
// onNext/onPrev when the release passes a distance or velocity threshold.
function useSwipeNavigation({
  ...
  return { dragOffset, isDragging, touchHandlers: { onTouchStart, onTouchMove, onTouchEnd } };
}
...
export default function WorkSlider({ images }: Props) {
  ...
  const { dragOffset, isDragging, touchHandlers } = useSwipeNavigation({
    containerRef,
    onNext: next,
    onPrev: prev,
  });
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-start min-h-0 select-none overflow-hidden">
      ...
      <div ref={containerRef} className="relative w-full flex-1 min-h-0">
        <div className="w-full h-full overflow-hidden touch-pan-y" {...touchHandlers}>
          <div
